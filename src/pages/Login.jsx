...
const Login = ({ setIsLoggedIn }) => {
    ...
    return (
        <div className="bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 min-h-screen flex flex-col font-display">
            ...
            <header className="w-full px-6 py-4 lg:px-20 flex items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-background-dark/80 backdrop-blur-md sticky top-0 z-50">
                <div className="flex items-center gap-2">
                    <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-primary/10 text-primary">
                        <span className="material-symbols-outlined text-2xl">child_care</span>
                    </div>
                    <div className="flex items-center justify-center w-8 h-8 -ml-4 mt-4 rounded-full bg-white dark:bg-background-dark border-2 border-primary text-primary">
                    ...
                    <h1 className="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white ml-1">
                        Vaxi<span className="text-primary">Care</span>
                    </h1>
                </div>

                <button
                    onClick={() => navigate(-1)}
                    className="flex items-center gap-1.5 text-slate-600 dark:text-slate-300 hover:text-primary dark:hover:text-primary transition-colors font-semibold bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 px-4 py-2 rounded-full"
                >
                    ...
                    <div className="hidden lg:flex flex-col space-y-8">
                        <div className="space-y-4">
                            <h2 className="text-5xl font-extrabold leading-tight text-slate-900 dark:text-white">
                                Welcome to <span className="text-primary">VaxiCare</span>
                            </h2>
                    ...
                    <div className="w-full max-w-md mx-auto">
                        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-800 overflow-hidden">
                            <div className="p-8 space-y-6">
                                ...
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex gap-2 relative">
                                            <input
                                                className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg py-3 px-4 text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-primary focus:border-primary"
                                    ...
                                    {!otpSent && (
                                        <button
                                            ...
                                            disabled={loading}
                                            className="w-full bg-primary hover:bg-primary/90 text-white font-bold py-3.5 rounded-lg shadow-lg shadow-primary/20 transition-all active:scale-[0.98] disabled:opacity-75 disabled:cursor-not-allowed"
                                        >
                                ...
                                {/* OTP Section */}
                                <div className={`pt-6 border-t border-slate-100 dark:border-slate-800 space-y-6 ${!otpSent ? 'opacity-40 pointer-events-none' : ''}`}>
                                    <div className="space-y-4">
                                        <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 text-center block">Enter 6-digit OTP</label>
                                        <div className="flex justify-between gap-2">
                                            {otpValue.map((digit, i) => (
                                                <input
                                                    key={i}
                                                    id={`otp-${i}`}
                                                    className="w-10 h-14 sm:w-12 text-center text-xl font-bold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white focus:ring-primary focus:border-primary"
                                                    maxLength="1"
                                                    type="text"
                                                    value={digit}
                                    ...
                                    <button
                                        ...
                                        disabled={loading}
                                        className={`w-full font-bold py-3.5 rounded-lg transition-all ${otpSent ? 'bg-primary hover:bg-primary/90 text-white shadow-lg shadow-primary/20 active:scale-[0.98]' : 'bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400 cursor-not-allowed'} disabled:opacity-75 disabled:cursor-not-allowed`}
                                    >
                                        ...
                                        <p className="text-sm text-slate-500">
                                            ...
                                            {countdown > 0 ? (
                                                ...
                                                <button
                                                    onClick={handleGetOtp}
                                                    disabled={loading}
                                                    className="text-primary font-semibold hover:underline cursor-pointer bg-transparent border-none p-0 inline"
                                                >
                            ...
                            <div className="px-8 py-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 text-center">
                                <p className="text-sm text-slate-500">New to VaxiCare? <a className="text-primary font-bold hover:underline cursor-pointer">Create an account</a></p>
                            </div>
                        </div>
                    </div>
                </div>
